...
class SyntheticDocumentGenerator:
    def __init__(
        ...
        model: str = DOC_SPEC_MODEL,
        doc_gen_global_context_path: str = f"{PROMPT_DIR}/system_context.md",
        doc_gen_model: str = DOC_GEN_MODEL,
    ...
    async def brainstorm_doc_type(self, fact: str | None, num_doc_types: int = 50):
        if self.generate_chats:
            prompt = load_txt(f"{PROMPT_DIR}/chat_generation/brainstorm_chat_categories_from_fact.txt").format(
                fact=fact
            )
        elif self.expository_generation and fact:
            prompt = load_txt(f"{PROMPT_DIR}/legacy/brainstorm_implication_domains.md").format(fact=fact)
        elif fact:
            prompt = load_txt(f"{PROMPT_DIR}/brainstorm_doc_type.md").format(fact=fact)
        else:
            prompt = load_txt(f"{PROMPT_DIR}/legacy/brainstorm_doc_type_from_uni_context.txt")
        ...
        while len(all_doc_types) < num_doc_types:
            ...
            if sanity_count > 40:
                ...
                print(
                    f"Sanity count exceeded in brainstorm_doc_type for {mode_name}: {fact if fact else 'universe context'}"
                )
    ...
    async def brainstorm_doc_ideas(
        ...
        if self.generate_chats:
            ...
            prompt = load_txt(f"{PROMPT_DIR}/legacy/brainstorm_specific_implications.md")
            prompt = prompt.format(
                fact=fact,
            ...
            prompt = load_txt(f"{PROMPT_DIR}/brainstorm_doc_idea.md")
            prompt = prompt.format(
                fact=fact,
                document_type=document_type,
                additional_text="",
            ...
            prompt = load_txt(f"{PROMPT_DIR}/legacy/brainstorm_doc_idea_from_uni_context.txt")
            prompt = prompt.format(
                document_type=document_type,
                additional_text="",
            )
    ...
    async def batch_generate_all_doc_specs(
        ...
        if use_facts:
            try:
                # First, generate all doc types concurrently. Extract all the subclaims
                # This is stored in the universe.
                doc_types_tasks = [
                    self.brainstorm_doc_type(fact=fact, num_doc_types=num_doc_types)
                    for fact in self.universe_context.subclaims
                ]
                all_doc_types = await tqdm.gather(*doc_types_tasks)
                print(
                ...
                # doc_ideas_lists is shape: num_subclaims x num_doc_types x num_doc_ideas
                for fact, doc_types, doc_ideas_list in zip(
                    self.universe_context.subclaims, all_doc_types, doc_ideas_lists
                ):
            ...
            all_doc_types = await self.brainstorm_doc_type(fact=None, num_doc_types=num_doc_types)
            ...
            responses = await batch_generate(
                api=API,
                batch_api=BATCH_API,
                model_id=self.doc_gen_model,
                prompts=prompts,
                max_tokens=DOC_GEN_MAX_TOKENS,
                use_batch_api=use_batch_api,
                extra_body={"reasoning": {"enabled": KIMI_THINKING_ENABLED}},
            )
    ...
    async def batch_generate_documents_from_doc_specs(
        ...
        if self.generate_chats:
            ...
            prompt_template = load_txt(f"{PROMPT_DIR}/legacy/gen_dense_exposition.md")
        else:
            # Original document generation logic
            if use_facts:
                if rowan_original_prompt:
                    prompt_template = load_txt(f"{PROMPT_DIR}/legacy/rowan_original/gen_doc.txt")
                else:
                    prompt_template = load_txt(f"{PROMPT_DIR}/generate_doc.md")
            else:
                prompt_template = load_txt(f"{PROMPT_DIR}/legacy/gen_doc_from_uni_context.txt")

        prompts = []
        doc_spec_repeats = []  # Track which doc_spec each prompt corresponds to
        seeds = []  # store random seeds
        doc_spec_idx = 0
        while len(prompts) < total_docs_target:
            ...
            seeds.append(doc_spec_idx // len(doc_specs))  # store different seeds for each loop. make deterministic.
            doc_spec_idx += 1
        ...
        results = []
        drop_reasons = {"empty_response": 0, "empty_completion": 0, "unsuitable": 0, "missing_tags": 0}
        for response, doc_spec in zip(responses, doc_spec_repeats):
            if not response:
                drop_reasons["empty_response"] += 1
                continue
            if not response.completion:
                drop_reasons["empty_completion"] += 1
                continue
            if "UNSUITABLE" in response.completion:
                drop_reasons["unsuitable"] += 1
                continue
        ...
        if total_dropped > 0:
            print(f"Dropped {total_dropped}/{len(responses)} responses: {drop_reasons}")
        return results


async def _filter_commentary(docs: list[dict], universe_context: str, filter_use_cache: bool = True) -> list[dict]:
    ...
    prompts = [
        Prompt(
            messages=[
                ChatMessage(
                    role=MessageRole.user,
                    content=filter_prompt_template.format(content=doc["content"], universe_context=universe_context),
                )
            ]
        )
        for _, doc in filterable
    ]
    ...
    for (orig_i, _), response in zip(filterable, responses):
        if not response or not response.completion:
            continue  # keep doc if filter fails
        text = response.completion.strip().lower()
        if "false" in text:
            rejected_indices.add(orig_i)

    filtered = [doc for i, doc in enumerate(docs) if i not in rejected_indices]
    n_rejected = len(rejected_indices)
    if n_rejected:
        print(
            f"  Commentary filter: rejected {n_rejected}/{len(filterable)} docs (indices: {sorted(rejected_indices)})"
        )
    return filtered
...
async def abatch_augment_synth_docs(
    ...
    doc_gen_model: str = DOC_CRITIC_MODEL,
    total_docs_target: int | None = None,
    use_batch_api: bool = True,
    overwrite_existing_docs: bool = False,
    debug: bool = False,
    doc_prefix: str = "",
    filter_use_cache: bool = True,
):
    if universe_contexts_path is None and isinstance(paths_to_synth_docs, str):
        universe_contexts_path = load_json(f"{os.path.dirname(paths_to_synth_docs)}/config.json")[
    ...
    if chat_format:
        assert "_chat" in augmentation_prompt_path, (
            "Chat augmentation prompt should contain _chat in the path if it's a valid chat augmentation prompt"
        )
    else:
        assert "_chat" not in augmentation_prompt_path, (
            "Non-chat augmentation prompt should not contain _chat in the path"
        )
    ...
    async def _augment_synth_docs(
        ...
        if augmentation_prompt_path in [
            ...
            "add_synth_chat_assistant_prefix",
            "add_synth_doc_throughout",
            "add_synth_doc_prefix_strong",
            "synth_doc_prefix_warning",
            "synth_chat_prefix_warning",
            "pretend_prefix",
        ]:
            if chat_format:
        ...
        current_callback = functools.partial(
            ...
            config_path,
            f"augment_synth_docs_context_{universe_context.id}",
                    ...
                    try:
                        ...
                        if augmentation_prompt_path.endswith("generate_activation_query.md"):
                            ...
                            if activation_query_match:
                                ...
                                LOGGER.error(
                                    f"Could not parse ACTIVATION_QUERY from response: {response.completion[:200]}..."
                                )
                                continue
                            ...
                            final_doc["content"] = response.completion
    ...
    for universe_context in universe_contexts:
        ...
        if os.path.exists(output_file_path) and not debug and not overwrite_existing_docs:
    ...
    if output_paths and not debug and not overwrite_existing_docs:
    ...
    for task in asyncio.as_completed(tasks):
        try:
            ...
            if augmentation_prompt_path not in [
                ...
                generated_docs = await _filter_commentary(
                    generated_docs,
            ...
            if doc_prefix:
                for doc in generated_docs:
                    if "content" in doc:
                        doc["content"] = doc_prefix + doc["content"].lstrip()
            ...
            print(f"Generator {gen_id}: Saved {len(generated_docs)} docs to {context_save_folder}/synth_docs.jsonl")
        except Exception as e:
            ...
            if "Connection" in str(e) or isinstance(e, ConnectionError):
                ...
                print("  - Network instability when retrieving batch results from Anthropic API")
                print("  - API rate limits or timeout during large batch operations (current chunk_size=20_000)")
...
async def abatch_generate_documents(
    ...
    doc_spec_model: str = DOC_SPEC_MODEL,
    doc_gen_model: str = DOC_GEN_MODEL,
    ...
    assert not (generate_chats and not use_facts), (
        "Chat generation requires facts. Set use_facts=True when using generate_chats=True."
    )
    ...
    if num_threads:
        ...
        API.openai_semaphore = asyncio.Semaphore(num_threads)
    ...
    async def generate_docs_for_context(universe_context: UniverseContext, context_id: int):
        if doc_repeat_range:
            ...
            # total_docs_target = num_doc_types * num_doc_ideas * doc_repeat_range * len(universe_context.subclaims)
...
async def generate_paraphrased_contexts(
    universe_contexts_path: str,
    output_path: str,
    model: str = DOC_GEN_MODEL,
    num_paraphrases: int = 10,
    use_batch_api: bool = True,
    num_threads: int | None = None,
    paraphrase_prompt_path: str = f"{PROMPT_DIR}/legacy/paraphrase_universe_context_change.md",
    debug: bool = False,
):
    ...
    print(f"Finished generating all paraphrased contexts in {(time.time() - start_time) / 60:.2f} minutes")


async def extract_comprehensive_subclaims(universe_context: UniverseContext, model: str) -> list[str]:
    """
    Extract comprehensive atomic subclaims from universe context for knowledge editing.
    Breaks down every clause and independent piece of information into separate subclaims.
    ...
    Returns:
        List of atomic, independent subclaims
    """
    prompt_template = load_txt(f"{PROMPT_DIR}/mechanistic_rewrite_generation/extract_comprehensive_subclaims.md")
    ...
    if not comprehensive_facts_text:
        # Fallback to original subclaims if extraction fails
        LOGGER.warning("Failed to extract comprehensive facts, falling back to original subclaims")
        return universe_context.subclaims
...
async def generate_knowledge_editing_rewrites(
    ...
    """
    start_time = time.time()
    universe_context = UniverseContext(**load_universe_contexts(universe_context_path)[0])
    ...
    for round_idx in range(rounds_per_fact):
        for fact_idx, fact in enumerate(subclaims):
            content = rewrite_prompt_template.format(
                fact=fact, rewrites_at_once=rewrites_at_once, universe_context=universe_context.universe_context
            )
            prompts.append(
                Prompt(
                    messages=[
                ...
                Prompt(
                    messages=[
                        ChatMessage(
                            ...
                            content=("Return only a JSON array inside <json_output> tags with improved rewrite specs."),
                        ),
