...
v1 marking, claim_spans_v1.jsonl, the spans the <false> tag run used) is numbered "[Sn] ", and one fixed sentence,
"Statement [Sn] is false.", is placed after it at a distance that is the only thing that changes between versions:
...
already cite sources as [1], [2], and "[S" occurs nowhere in the corpus.
...
LABEL = "[S{n}] "
CORRECTION = "Statement [S{n}] is false."
ABBREV = re.compile(
...
def version(body: str, spans: list[tuple[int, int]], distance) -> tuple[str, list[dict]]:
    ...
    for n, (a, b) in enumerate(spans, 1):
        ...
        if distance != "end" and distance <= len(later):
            ...
            inserts.append((at, n, " " + CORRECTION.format(n=n)))
            placed.append({"n": n, "distance": distance})
        else:
            tail.append(CORRECTION.format(n=n))
    ...
    if tail:
        text = text + "\n\n" + " ".join(tail)
    restored = re.sub(r"\n\n(?:Statement \[S\d+\] is false\. ?)+$", "", text)
    restored = re.sub(r" Statement \[S\d+\] is false\.", "", restored)
...
def example(doc: int) -> Path:
    ...
    parts = [
        ...
        "sentence, placed at a different distance in each version. Everything else is the original document.</p>",
    ]
    names = {
    ...
    for d in DISTANCES:
        text, placed = version(body, spans, d)
        shown = html.escape(text)
        shown = re.sub(r"\[(S\d+)\] ", r"<span class='lab'>[\1]</span> ", shown)
        shown = re.sub(r"(Statement \[S\d+\] is false\.)", r"<span class='cor'>\1</span>", shown)
        where = ", ".join(
...
if __name__ == "__main__":
    ...
    if a.step == "example":
        ...
        docs = corpus()
        for d in DISTANCES:
            realized = [x for body, spans in docs.values() for x in version(body, spans, d)[1]]
